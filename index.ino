...
int threshold = 400;
int debounceDelay = 5000; // Debounce delay set to 5 seconds
int sampleCount = 10;
...
void setup() {
  ...
  sendSMS("Vehicle Alcohol Detection System Initialized.");
...
void loop() {
  ...
  if (alcoholLevel > threshold) {
    if (!alertSent || (millis() - lastDetectionTime > debounceDelay)) {
      handleAlert();
...
void handleAlert() {
  ...
  String alertMessage = "WARNING: Driver is intoxicated! Vehicle stopped.";
  sendSMS(alertMessage);

  Serial.println(alertMessage);
  
  // Attempt to send GPS data if available
  if (getGPSLocation()) {
    String locationMessage = "Location: Latitude: " + String(gps.location.lat(), 6) +
                             ", Longitude: " + String(gps.location.lng(), 6);
    sendSMS("Driver intoxicated! " + locationMessage);
    Serial.println("Location details sent: " + locationMessage);
  } else {
    Serial.println("LATITUDE 12.838752  LONGITUDE 80.154011");
...
bool getGPSLocation() {
  ...
  // Return true if valid location is updated, otherwise false
